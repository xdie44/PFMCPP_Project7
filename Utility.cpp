...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    {
        chDwarf->boostArmor( int(item.getBoost() * 1.1) );
    }
    else if( auto* chPaladin = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        chPaladin->boostArmor( int(item.getBoost() * 1.3) );
    }
    else if( auto* chDragonSl = dynamic_cast<DragonSlayer*>(character))
    {
        chDragonSl->boostArmor( int(item.getBoost() * 1.5) );
    }
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        chPaladin->boostHitPoints(int(item->getBoost() * 1.5) );
    }
    else if( auto* chDragonSl = dynamic_cast<DragonSlayer*>(character))
    {
        chDragonSl->boostHitPoints(int(item->getBoost() * 1.25) );
    }
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* chDwarf = dynamic_cast<Dwarf*>(character) )
    {
        chDwarf->boostAttackDamage(int(item->getBoost() * 1.5) );
    }
    else if( auto* chPaladin = dynamic_cast<Paladin*>(character) )
    {
        chPaladin->boostAttackDamage(int(item->getBoost() * 1.33) );
    }
    else if( auto* chDragonSl = dynamic_cast<DragonSlayer*>(character))
    {
        //assert(false);
        ...
        chDragonSl->boostAttackDamage(item->getBoost() * chDragonSl->getAttackDamage() );
    }
    else if( auto* chDragon = dynamic_cast<Dragon*>(character) )
    {
